...
muteDevice(*)
{
	...
	If mute=1
	{
		...
		scLV.Modify(iRow,'Icon2',,,,,0)
		Notify.show({BDText:Dev " set to unmute",GenIcon:IconUnmute,HDFontColor:0x23B14D})
	...
	Else
	{
		...
		scLV.Modify(iRow,"Icon1",,,,,1)
		Notify.show({BDtext:Dev " set to mute",GenIcon:IconMute,HDFontColor:"Red"})
	...
	loop
	{
		...
		loop
		{
			...
			&& scGui['MicFilter'].value
				scLV.Add("Icon" (mute?1:2) , component, dev, devName, vol, mute)
			else if scGui['MicFilter'].value = 0
				scLV.Add("Icon" (mute?1:2) , component, dev, devName, vol, mute) ; + (component ~= "i)Mic|Cap" ? 0:2) 
