...
TryUpdate() {
	...
	UpdateText('Ctrl_Freeze', IsElevated ? 'Press Right Control when ' pName ' is not Active' : '(Press Right Control to update)')
	...
	SB.SetText(CheckExeBitness(processpath),3)
}
...
class Border extends Highlight {
	__New(winTitle, size:=2, color:='Red', flash:=false)
	{
		...
		border := Gui('-Caption +ToolWindow -DPIScale')
